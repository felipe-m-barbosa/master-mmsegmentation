...
class MotionAwareCropSelection(object):
    ...
    def __call__(self, results):
        ...
        extract_magnitude = lambda f: torch.sqrt(torch.as_tensor(f[..., 0])**2+torch.as_tensor(f[..., 1])**2 if f is not None else torch.zeros(results['optflows'][0].shape[:-1])) # computes magnitude from optical flow
        ...
        patches_mags = patches_mags[0] # the list is used just to access the result of the map function. We can then extract it by recovering the only element of the returned list.
        # print("PATCHES_MAGS: ", patches_mags.shape)
